...
export(String) var record_action := ""
export(String) var save_action := ""
...
func _input(event: InputEvent):
	if event.is_action_pressed(record_action):
		...
		if (not _running): # It was running before
			_label.show()
			_label.text = "Waiting for capture...\nNumber of frames recorded: " + str(_images.size())
	if event.is_action_pressed(save_action) and not _running:
		if (use_thread):
